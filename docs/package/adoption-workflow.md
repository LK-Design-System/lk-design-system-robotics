...
`LK-Design-System/lk-design-system@lds-v0.2.13`.
...
전환 중 shared component, token, asset, pattern의 변경 필요가 발견되어도 제품 전환 요청이 그 변경 권한을 자동으로 부여하지 않습니다. 해당 변경은 [컴포넌트 워크플로](./shared/policies/COMPONENT_WORKFLOW.md), [토큰 거버넌스](./shared/policies/TOKEN_GOVERNANCE.md), 저장소의 scope escalation gate를 따로 적용합니다.
...
2. [Foundation index](./shared/foundations/README.md)에서 관련 원리와 선택 기준을 읽습니다. 단일 AI context가 필요하면 [Foundation LLM bundle](./shared/foundations/llms.txt)을 대신 사용합니다.
3. 관련 CSS 이름과 설명을 [token source](./shared/token-reference.json)에서 확인합니다. generated CSS 값만 보고 의미를 추론하지 않습니다.
4. 후보 component를 정한 뒤 [component index](./shared/components/README.md)와 해당 targeted guide를 읽습니다. 전체 [component LLM bundle](https://github.com/LK-Design-System/lk-design-system/blob/lds-v0.2.13/docs/components/llms.txt)은 retrieval/indexing 용도이며 매 작업에서 통째로 읽는 필수 입력이 아닙니다.
...
- [docs/TOKEN_GOVERNANCE.md](./shared/policies/TOKEN_GOVERNANCE.md)
- [docs/PACKAGE_MIGRATION_GUIDE.md](./shared/policies/PACKAGE_MIGRATION_GUIDE.md)
- [docs/foundations/design-token.md](./shared/foundations/design-token.md)
- [docs/foundations/color.md](./shared/foundations/color.md)
- [tokens/source.json](./shared/token-reference.json)
...
- [docs/foundations/typography.md](./shared/foundations/typography.md)
- [docs/foundations/layout.md](./shared/foundations/layout.md)
- [docs/foundations/spacing.md](./shared/foundations/spacing.md)
- [docs/foundations/radius.md](./shared/foundations/radius.md)
- [docs/foundations/elevation.md](./shared/foundations/elevation.md)
- [docs/foundations/gradient.md](./shared/foundations/gradient.md)
- [docs/foundations/aspect-ratio.md](./shared/foundations/aspect-ratio.md)
...
- [docs/foundations/state.md](./shared/foundations/state.md)
- [docs/foundations/motion.md](./shared/foundations/motion.md)
- [docs/LOADING_PATTERN.md](./shared/patterns/loading.md)
...
- [docs/foundations/iconography.md](./shared/foundations/iconography.md)
- [packages/core/assets/icons/manifest.json](./shared/assets/icons/manifest.json)
...
- [docs/foundations/writing.md](./shared/foundations/writing.md)
- [docs/foundations/voice-and-tone.md](./shared/foundations/voice-and-tone.md)
- [docs/foundations/international-design.md](./shared/foundations/international-design.md)
- [docs/COPY_REVIEW_CONTRACT.md](./shared/policies/COPY_REVIEW_CONTRACT.md)
...
- [docs/foundations/inclusive-design.md](./shared/foundations/inclusive-design.md)
- [docs/ACCESSIBILITY_CONTRACTS.md](./shared/policies/ACCESSIBILITY_CONTRACTS.md)
...
- [docs/components/README.md](./shared/components/README.md)
- [docs/PACKAGE_MIGRATION_GUIDE.md](./shared/policies/PACKAGE_MIGRATION_GUIDE.md)
- [docs/COMPONENT_WORKFLOW.md](./shared/policies/COMPONENT_WORKFLOW.md)
...
검증 가능한 attestation은 [`LDS_UI_ADOPTION_REPORT.schema.json`](./shared/adoption-report.schema.json)을 따릅니다. 한 report는 scope와 하나 이상의 surface를 가지며, 각 surface에는 다음 항목이 모두 있어야 합니다.
...
`adoption-checklist.json`은 읽기 전용 계약입니다. 이 파일을 수정하지 않습니다. [Schema-valid report template](./shared/adoption-report.example.json)을 소비 저장소의 `.lds/adoption-report.json`으로 복사할 때, template의 상대 `$schema`가 가리키는 [report schema](./shared/adoption-report.schema.json)도 같은 디렉터리에 그 basename 그대로 함께 복사합니다. Placeholder를 실제 outcome과 evidence로 모두 교체하며, copied schema는 pinned LDS revision에서 온 read-only 입력으로 관리합니다.

소비 저장소에 [config schema](./shared/adoption-config.schema.json)를 따르는 `.lds/adoption.config.json`을 둡니다. 기본 report 경로는 `.lds/adoption-report.json`이며 config의 `reportDirectory`와 CLI `--report`로 명시적으로 바꿀 수 있습니다.
...
에이전트로 UI를 조립하는 소비 저장소는 셋업 단계에서 [consumer agent skill](./shared/agent-skills/lds-ui/SKILL.md)도 함께 설치합니다 — 규칙이 조립 시점에 push 로드되는 채널입니다. Claude Code는 설치된 `@lk-design-system/lds-core/docs/agent-skills/lds-ui/`를 `.claude/skills/lds-ui/`로 복사하고, AGENTS.md 기반 에이전트(Codex 등)는 SKILL.md 설치 섹션의 라우팅 블록을 저장소 AGENTS.md에 추가합니다. `check-adoption`은 둘 다 없으면 `AGENT_SKILL_MISSING` 경고를 출력합니다(advisory — 판정을 차단하지는 않습니다).
