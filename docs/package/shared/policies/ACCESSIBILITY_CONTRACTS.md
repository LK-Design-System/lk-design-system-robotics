...
| AlarmCaseBanner | root는 `section[aria-labelledby]`이며 제목이 이름을 소유한다. 미확인 사례만 숨김 `role="alert"` 문장으로 초점 이동 없이 알리고 그 밖의 lifecycle은 `role="status"`다(`announce={false}`로 제품 live region에 양보 가능). 심각도·lifecycle·신선도·연결·권한·홍수는 색이 아니라 텍스트 배지로 읽히고, 발생 시각은 `<time dateTime>`이다. 확인 button은 차단 시 `aria-describedby`로 사유를 연결하고 pending 중 중복 실행을 막으며, 권한이 없어도 사례 자체는 숨기지 않는다. 확인 기록의 빈 칸은 `기록 없음` 텍스트와 `data-missing`으로 남는다. |
| MissingValue | 값이 없는 자리는 비워 두지 않는다. 보이는 em dash는 `aria-hidden`이고 보조기술에는 `값 없음`(해당 없음이면 `해당 없음`)을 visually hidden 텍스트로 제공해, 낭독기마다 다른 빈 셀·대시 처리에 의존하지 않는다. 결측·빈 문자열·숫자 `0`·loading·error는 같은 표시로 합치지 않으며, 목록 전체의 공백은 셀이 아니라 `EmptyState`·`ResourceState`가 소유한다. 색은 AA를 충족하는 `label-alternative`이며 색만으로 결측을 전달하지 않는다. |
| DotMatrixPreview | canvas는 `role="img"`이며 accessible name에 패널 이름·표시 문구·밝기 %를 싣고 같은 문구를 fallback content와 요약 줄 텍스트로 반복한다. 켜진 점이 없으면 `emptyLabel`을 읽고 `data-empty`를 둔다. 상호작용 요소가 없어 Tab stop을 만들지 않으며 상태는 `status` 슬롯의 LDS 배지가 라벨로 전달한다. |
| ScheduleCalendar | `role="grid"`·`row`·`columnheader`·`gridcell`의 APG data grid이며 날짜/시간 slot button 하나만 roving tabindex로 Tab에 남는다. Arrow는 일·주(월) 또는 시간·일(주/일), Home/End는 주의 시작/끝 또는 첫/마지막 시간, PageUp/PageDown은 기간 이동, Enter/Space는 `onSlotSelect`다. 이벤트 chip은 일반 button이며 accessible name에 제목과 날짜·시간 범위를 붙이고 톤 색만으로 의미를 전달하지 않는다. 기간 제목은 `aria-live="polite"`, 현재 시각 선은 `aria-hidden`이다. |
